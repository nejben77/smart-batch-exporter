...
* method for details.
*/
var DataModelObject = class DataModelObject {
	/** @internal */
	...
	get parent() {
		...
		return handle ? this.objectRegistry.getObjectFromHandle(handle, DataModelObject) : null;
	}
};
//#endregion
//#region src/transaction.ts
/**
* Invokes an asynchronous data model function inside a transaction, returning
* a promise that resolves when its `onResult` callback fires (or rejects when
* `onError` is invoked).
*
*  @internal
*/
const invokeAsync = (dataModel, fn, ...args) => new Promise((resolve, reject) => {
	dataModel.withinTransaction(() => fn(...args, resolve, reject));
});
/**
* Like {@link invokeAsync}, but for data model functions whose `onResult`
* yields a {@link Handle}. The handle is wrapped as an instance of `T` via
* the object registry before the promise resolves.
*
* @internal
*/
const createAsync = (dataModel, registry, type, fn, ...args) => new Promise((resolve, reject) => {
...
//#region src/Track.ts
/** Represents a track. */
var Track = class Track extends DataModelObject {
	...
	get groupTrack() {
		...
		return handle ? this.objectRegistry.getObjectFromHandle(handle, Track) : null;
	}
	get devices() {
...
//#endregion
//#region src/DataModelObjectRegistry.ts
...
//#endregion
//#region src/Ui.ts
const toProgressOptions = (text, progress) => typeof progress === "number" ? {
	progress,
	text
} : { text };
...
*/
const initialize = (context, apiVersion) => {
...
*/
let GridQuantization = /* @__PURE__ */ function(GridQuantization) {
	GridQuantization[GridQuantization["NoGrid"] = 0] = "NoGrid";
	GridQuantization[GridQuantization["EightBars"] = 1] = "EightBars";
	GridQuantization[GridQuantization["FourBars"] = 2] = "FourBars";
	GridQuantization[GridQuantization["TwoBars"] = 3] = "TwoBars";
	GridQuantization[GridQuantization["Bar"] = 4] = "Bar";
	GridQuantization[GridQuantization["Half"] = 5] = "Half";
	GridQuantization[GridQuantization["Quarter"] = 6] = "Quarter";
	GridQuantization[GridQuantization["Eighth"] = 7] = "Eighth";
	GridQuantization[GridQuantization["Sixteenth"] = 8] = "Sixteenth";
	GridQuantization[GridQuantization["ThirtySecond"] = 9] = "ThirtySecond";
	return GridQuantization;
}({});
/** Warp algorithm for an audio clip. Corresponds to Live's Warp Mode chooser. */
let WarpMode = /* @__PURE__ */ function(WarpMode) {
...
//#endregion
export { Application, AudioClip, AudioTrack, Chain, ChainMixer, Clip, ClipSlot, Commands, CuePoint, DataModelObject, Device, DeviceParameter, DrumChain, DrumRack, EXTENSIONS_API_VERSIONS, Environment, GridQuantization, MidiClip, MidiTrack, RackDevice, Resources, Sample, Scene, Simpler, Song, TakeLane, Track, TrackMixer, Ui, WarpMode, initialize };
